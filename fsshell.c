...
int displayFiles(fdDir *dirp, int flall, int fllong)
{
	...
	while (di != NULL)
	{
		printf("displayFiles inside while loop\n");
		if ((di->d_name[0] != '.') || (flall)) // if not all and starts with '.' it is hidden
		{
			printf("displayFiles inside if(di->d_name[0]\n");
			if (fllong)
			{
...
int cmd_ls(int argcnt, char *argvec[])
{
	...
	if (optind < argcnt)
	{
		...
		for (int k = optind; k < argcnt; k++)
		{
			...
			{
				printf("inside cmd_ls if(fs_isDir)\n");
				fdDir *dirp;
	...
	{
		printf("fs_getcwd called\n");
		char *path = fs_getcwd(cwd, DIRMAX_LEN); // get current working directory
		fdDir *dirp;
		dirp = fs_opendir(path);
		printf("cmd_ls inside else after fs_opendir before calling displayFiles");
